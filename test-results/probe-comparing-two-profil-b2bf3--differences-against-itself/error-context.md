...
- Name: probe.spec.ts >> comparing two profiles reports no differences against itself
- Location: e2e\probe.spec.ts:99:1
...
> 102 |   await expect(page.locator('#out pre')).toBeVisible({ timeout: 110_000 });
  ...
  114 |   expect(text).toContain('Every format behaves identically');
  115 | });
  116 | 
```